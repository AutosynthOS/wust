...
export function fmtVreg(id: number): string { return `v${id}`; }
...
export function fmtRegInst(inst: unknown): string {
	if (typeof inst !== 'object' || inst === null) return String(inst);
	if ('Define' in inst) {
		const d = (inst as { Define: { vreg: number; value: unknown } }).Define;
		const val = d.value;
		if (val === 'InstDst') return `${fmtVreg(d.vreg)} = <pending>`;
		if (typeof val === 'object' && val !== null) {
			if ('Const' in val) return `${fmtVreg(d.vreg)} = #${(val as { Const: number }).Const}`;
			if ('PReg' in val) return `${fmtVreg(d.vreg)} = ${fmtPreg((val as { PReg: number }).PReg)}`;
		}
		return `define ${fmtVreg(d.vreg)}`;
	}
	if ('SetSlot' in inst) {
		const s = (inst as { SetSlot: { vreg: number; slot: { base: number; offset: number } } }).SetSlot;
		return `set [${fmtPreg(s.slot.base)}+${s.slot.offset}] <- ${fmtVreg(s.vreg)}`;
	}
	if ('ClearSlot' in inst) {
		const s = (inst as { ClearSlot: { vreg: number; slot: { base: number; offset: number } } }).ClearSlot;
		return `clear [${fmtPreg(s.slot.base)}+${s.slot.offset}] ${fmtVreg(s.vreg)}`;
	}
	if ('Clobber' in inst) {
		return `clobber ${fmtVreg((inst as { Clobber: { vreg: number } }).Clobber.vreg)}`;
	}
	if ('Resolve' in inst) {
		return `resolve ${fmtVreg((inst as { Resolve: { vreg: number } }).Resolve.vreg)}`;
	}
	return JSON.stringify(inst);
}

export function fmtIrInst(inst: unknown): string {
	if (inst === 'Return') return 'ret';
	if (typeof inst !== 'object' || inst === null) return String(inst);
	if ('Alu' in inst) {
		const a = (inst as { Alu: { op: unknown; dst: number; lhs: number; rhs: number } }).Alu;
		return `${fmtVreg(a.dst)} = ${fmtAluOp(a.op)} ${fmtVreg(a.lhs)}, ${fmtVreg(a.rhs)}`;
	}
	if ('Call' in inst) {
	...
	if ('BrIf' in inst) {
		const b = (inst as { BrIf: { cond: number; block_if: unknown; block_else: unknown } }).BrIf;
		return `br_if ${fmtVreg(b.cond)} then ${fmtBlockId(b.block_if)} else ${fmtBlockId(b.block_else)}`;
	}
	if ('Skipped' in inst) {
		return `~${fmtIrInst((inst as { Skipped: unknown }).Skipped)}`;
	}
	if ('Load' in inst) {
...
function convertRegionSnapshot(regions: { label: string; slots: number[] }[]): RegionSnapshotView[] {
	return regions.map(r => ({
		label: r.label,
		slots: r.slots.map(fmtVreg),
	}));
}
...
function convertRegAllocState(state: {
	...
	const bindings = state.bindings.map((vreg, i) => ({
		preg: fmtPreg(i),
		vreg: vreg !== null ? fmtVreg(vreg) : null,
	}));
	...
	for (let i = 0; i < state.entries.length; i++) {
		...
		const loc = entry.loc;
		if (loc === 'Pending') {
			vreg_locs.push({ vreg: fmtVreg(i), loc: 'pending' });
		} else if (loc === 'Mem') {
			vreg_locs.push({ vreg: fmtVreg(i), loc: 'mem' });
		} else if (typeof loc === 'object' && loc !== null && 'Const' in loc) {
			vreg_locs.push({ vreg: fmtVreg(i), loc: 'const' });
...
export function transformTrace(raw: any[]): FunctionView[] {
	...
	// Pre-pass: collect regalloc snapshots and ASM, keyed by ir_index/parent
	...
	for (const e of raw) {
		...
		switch (e.type) {
			case 'function_start': {
				currentFunc = { index: e.index, vreg_defs: [], regions: [], blocks: [] };
			...
			case 'build_end': {
				...
				currentFunc.vreg_defs = e.vreg_defs.map((d: { id: number; width: string; target: number | null }) => ({
					id: fmtVreg(d.id),
			...
			case 'reg': {
				if (!currentGroup && currentBlock) {
					currentGroup = { pc: null, label: currentBlock.groups.length === 0 ? '(prologue)' : '(internal)', ops: [] };
					currentBlock.groups.push(currentGroup);
				}
				...
				currentGroup.ops.push({
					...
					kind: 'reg',
					inst: e.inst,
					text: fmtRegInst(e.inst),
					asm: asmByParent.get(irIdx) ?? [],
					region_snapshot: e.regions ? convertRegionSnapshot(e.regions) : null,
...
export function renderText(functions: FunctionView[]): string {
	...
	for (const func of functions) {
		...
		w(`=== func ${func.index} — ${func.vreg_defs.length} vregs, ${func.blocks.length} blocks, ${nAsm} asm ===`);
		w('');
