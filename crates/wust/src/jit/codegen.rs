...
use crate::Module;
use crate::jit::{compiler, lower_aarch64};
...
impl CodegenOutput {
    ...
    pub fn render_inspect(&self) -> String {
        ...
        for (name, func) in &self.functions {
            ...
            // Build regions from markers. Each consecutive pair of markers
            // defines a region. The last region extends to the end of code.
            let markers = &func.markers;
            let total_words = func.code.len();

            // Region labels: marker 0 = prologue, 1..=N = IR insts, N+1 = yield, N+2 = completion.
            let ir_lines: Vec<&str> = func.ir.lines().collect();
            // First line is "fn(...):"; IR instructions follow.
            // We'll use them as labels for the IR regions.
            ...
            for region_idx in 0..markers.len() {
                ...
                // Determine the label for this region.
...
fn region_label<'a>(region_idx: usize, ir_inst_count: usize, ir_lines: &[&'a str]) -> &'a str {
    ...
    if ir_idx < ir_inst_count {
        // IR instructions start at line 1 (line 0 is "fn(...):")
...
fn disasm(word: u32) -> String {
    ...
    // All share imm7 at bits[21:15], scaled by 8 for 64-bit.
    ...
    // ADD x, immediate (reg 31 = SP in this encoding)
    ...
    // SUB x, immediate (reg 31 = SP in this encoding)
    ...
    // CSINC w (covers CSET: CSINC wd, wzr, wzr, invert(cond))
